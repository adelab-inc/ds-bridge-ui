...
export class FigmaApiError extends Error {
  constructor(
    message: string,
    public readonly code: 'AUTH_ERROR' | 'NOT_FOUND' | 'RATE_LIMITED' | 'SERVER_ERROR',
...
export async function fetchFigmaNodes(
  ...
  if (!response.ok) {
    ...
    if (status === 401 || status === 403) {
      throw new FigmaApiError('Figma 인증 실패: 토큰을 확인해주세요', 'AUTH_ERROR', status);
    }

    if (status === 404) {
      throw new FigmaApiError('파일 또는 노드를 찾을 수 없습니다', 'NOT_FOUND', 404);
    }

    if (status === 429) {
      const retryAfterHeader = response.headers.get('Retry-After');
      const retryAfter = retryAfterHeader ? parseInt(retryAfterHeader, 10) : undefined;
      throw new FigmaApiError('Figma API rate limit 초과', 'RATE_LIMITED', 429, retryAfter);
    }
    ...
    throw new FigmaApiError(`Figma API 요청 실패: ${status}`, 'SERVER_ERROR', status);
  }
