...
export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ room_id: string; id: string }> }
) {
  try {
      ...
      return NextResponse.json(
        {
          detail: [
            {
              loc: ['path'],
              msg: 'room_id and id are required',
    ...
    const aiResponse = await fetch(
      `${aiServerUrl}/description/${room_id}/versions/${id}`,
      {
        method: 'GET',
        headers: {
          'X-API-Key': xApiKey,
    ...
    if (!aiResponse.ok) {
      if (aiResponse.status === 404) {
        return NextResponse.json(
          { error: 'Description version not found' },
          { status: 404 }
        );
      }
