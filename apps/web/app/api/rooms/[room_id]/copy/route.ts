...
/**
 * POST /api/rooms/{room_id}/copy — 방을 대상 유저에게 복제
 *
 * 삭제(DELETE)와 동일한 제로트러스트 소유권 검증 → 브라우저의 Supabase access
 * token을 `Authorization: Bearer`로 백엔드에 forward해야 한다. 본인 소유 방이
 * 아니면 백엔드가 403을 반환한다.
 *
 * 성공 시 백엔드 status(201)와 새로 생성된 방 정보를 그대로 패스스루한다.
 */
export async function POST(
  ...
  try {
    ...
    const aiResponse = await fetch(`${aiServerUrl}/rooms/${room_id}/copy`, {
      method: 'POST',
      headers: {
