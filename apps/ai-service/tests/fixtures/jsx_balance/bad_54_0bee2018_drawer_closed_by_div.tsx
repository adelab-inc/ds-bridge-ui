...
import { GridLayout, RowPattern, RowSlot, TitleSection, Button, Field, Select, Tab, Badge, Icon, IconButton, Radio, Option, OptionGroup, Divider, Tag, DataGrid, COLUMN_TYPES, Alert, Segment, Checkbox } from '@/components';
...
const PushSend = () => {
  ...
  return (
    <Drawer open={true} onClose={() => window.history.back()} size="xl">
          ...
          <div className="col-span-4 flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3">
                <OptionGroup label="전송 시점" showLabel={true} orientation="horizontal">
                  <Option label="즉시전송" size="md">
                    <Radio 
                      value={sendTime === 'immediate' ? 'checked' : 'unchecked'} 
                      onChange={() => setSendTime('immediate')} 
                    />
                  </Option>
                  <Option label="예약전송" size="md">
                    <Radio 
                      value={sendTime === 'reserved' ? 'checked' : 'unchecked'} 
                ...
                {sendTime === 'reserved' && (
                  <div className="flex flex-col gap-3 p-4 bg-canvas rounded-lg border border-default">
                    <Field 
                      type="date" 
                      label="예약일자" 
                      showLabel={true} 
          ...
          <div className="col-span-8 flex flex-col gap-8">
            <div className="flex flex-col gap-4">
              ...
              <div className="flex gap-4">
                {/* 카테고리 메뉴 */}
                <div className="w-40 flex flex-col border border-default rounded-lg bg-surface overflow-hidden h-fit">
                  {categories.map((cat) => (
                    <div key={cat.id} className="flex flex-col border-b border-default last:border-b-0">
                      <button
                        ...
                        className={`px-3 py-3 text-sm text-left transition-colors flex items-center justify-between ${
                      ...
                      {category === cat.id && (
                        <div className="bg-surface py-1 flex flex-col">
                          {cat.sub.map((sub) => (
                            <button
                              key={sub}
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {templates
                      ...
                      .map((tpl) => (
                        ...
                        >
                          <div className="p-2 border-b border-default bg-canvas/30">
                            <span className="text-xs font-medium text-secondary">{tpl.title}</span>
                          </div>
                          <div className={`p-3 flex-1 ${subTab === 'short' ? 'min-h-[120px]' : 'min-h-[180px]'}`}>
                            <p className={`text-sm text-primary leading-relaxed ${subTab === 'short' ? 'line-clamp-4' : 'line-clamp-[8]'}`}>
                              {tpl.body}
                            </p>
                          </div>
                          <div className="p-2 border-t border-default flex justify-between items-center bg-canvas/10">
                            <span className="text-xs text-accent font-bold">{tpl.byte} byte</span>
                            <span className="text-xs text-secondary">{subTab === 'short' ? '단문' : '장문'}</span>
                          </div>
                        </div>
                      ))}
                  </div>
            ...
            <div className="flex flex-col gap-6">
              <div className="flex items-center gap-6">
                <h3 className="text-sm font-semibold text-primary min-w-[80px]">메시지 유형</h3>
                <div className="flex flex-wrap gap-x-6">
                  <Option label="공지" size="md">
                    <Radio value={pushType === 'notice' ? 'checked' : 'unchecked'} onChange={() => setPushType('notice')} />
                  </Option>
                  <Option label="사업단" size="md">
                    <Radio value={pushType === 'org' ? 'checked' : 'unchecked'} onChange={() => setPushType('org')} />
                  </Option>
                  <Option label="시책" size="md">
                    <Radio value={pushType === 'policy' ? 'checked' : 'unchecked'} onChange={() => setPushType('policy')} />
                  </Option>
                  <Option label="경조사" size="md">
                    <Radio value={pushType === 'event' ? 'checked' : 'unchecked'} onChange={() => setPushType('event')} />
                  </Option>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-4">
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <h3 className="text-sm font-semibold text-primary">발송대상</h3>
                    <Badge type="count" label={recipients.length} status="info" appearance="subtle" />
                  </div>
      ...
      <Drawer.Footer>
        <Button buttonType="tertiary" size="lg" label="미리보기" onClick={() => {}} />
        <Button buttonType="primary" size="lg" label="발송하기" onClick={() => {}} />
      </Drawer.Footer>
