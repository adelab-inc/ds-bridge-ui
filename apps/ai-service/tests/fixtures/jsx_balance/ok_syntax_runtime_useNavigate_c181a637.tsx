...
import { GridLayout, RowPattern, RowSlot, TitleSection, Button, Field, Select, Tab, Badge, Icon, IconButton, Radio, Option, OptionGroup, Divider, Tag, DataGrid, COLUMN_TYPES, Alert, Segment } from '@/components';
...
const PushSend = () => {
  ...
  const addRecipient = () => {
    setRecipients([...recipients, { id: Date.now(), name: '', phone: '' }]);
  };
  ...
  return (
    <div className="bg-canvas p-8">
      <GridLayout type="C">
        {/* [좌측 영역] 메시지 작성 및 발송 패널 (Sticky) */}
        <div className="sticky top-8 h-fit">
          <TitleSection title="보플PUSH 발송" favorite={false} onFavoriteChange={() => {}} showBreadcrumb={false}>
            <Button buttonType="tertiary" size="sm" label="닫기" onClick={() => navigate(-1)} />
          </TitleSection>
          ...
          <div className="mt-5 flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Alert
                type="info"
                body="수신자/발신자명을 넣고자 하는 부분에 {수신자명}, {발신자명}로 입력하시면 됩니다."
              />
            </div>
            ...
            <div className="flex gap-2 pt-2">
              <Button buttonType="tertiary" size="lg" label="미리보기" className="flex-1" />
              <Button buttonType="primary" size="lg" label="발송하기" className="flex-1" />
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-6">
          {/* 상단 안내 및 탭 (Sticky 고정) */}
          <div className="sticky top-8 z-10 flex flex-col gap-2 bg-canvas pb-2">
          ...
          <div className="flex gap-6">
            {/* 카테고리 메뉴 */}
            <div className="w-48 flex flex-col border border-default rounded-lg bg-surface overflow-hidden h-fit">
              {categories.map((cat) => (
            ...
            <div className="flex items-center gap-8">
              ...
              <div className="flex flex-wrap gap-x-10">
                ...
                <Option label="인포DB" size="lg">
                  <Radio value={pushType === 'infodb' ? 'checked' : 'unchecked'} onChange={() => setPushType('infodb')} />
                </Option>
                ...
                <Option label="회장님" size="lg">
                  <Radio value={pushType === 'chairman' ? 'checked' : 'unchecked'} onChange={() => setPushType('chairman')} />
                </Option>
              </div>
            </div>
          </div>
              ...
              <RowSlot slot="actions">
                <div className="flex justify-between items-center w-full mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge type="count" label={recipients.length} appearance="solid" status="info" />
                  </div>
                  <div className="flex gap-2">
                    <Button buttonType="tertiary" size="sm" label="직접입력(외부)" startIcon={<Icon name="add" size={16} />} showStartIcon={true} />
                    <div className="flex border border-default rounded-lg overflow-hidden">
                      <IconButton iconOnly={<Icon name="add" size={20} />} size="md" iconButtonType="ghost" aria-label="추가" onClick={addRecipient} />
                      <Divider orientation="vertical" />
                      <IconButton iconOnly={<Icon name="minus" size={20} />} size="md" iconButtonType="ghost" aria-label="삭제" onClick={removeRecipient} />
                    </div>
                  </div>
                </div>
              </RowSlot>
