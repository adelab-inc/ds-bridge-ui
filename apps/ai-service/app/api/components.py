...
# Part 4. API 설계 (백엔드 참고용)

> 코드에서 API 호출, 데이터 흐름, 조회/저장 동작이 확인되는 경우 백엔드 개발에 필요한 API 명세를 작성합니다.
...
17. Part 4(API 설계)는 백엔드 개발자 참고용입니다. 화면의 버튼 동작, 조회/저장 흐름, 필터 파라미터, 그리드 컬럼에서 필요한 API를 추론하세요. 엔드포인트 경로는 RESTful 컨벤션을 따르되, 실제 구현과 다를 수 있음을 전제합니다. 요청/응답 필드명은 코드의 state, columnDefs, API 호출 파라미터에서 추론하세요
