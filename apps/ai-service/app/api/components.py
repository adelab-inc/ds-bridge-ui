...
> [중요] 모든 선택형 항목에 대해 각각 코드 목록을 작성하세요. 필터바, 폼, 다이얼로그 등 화면 전체의 드롭다운/라디오를 빠짐없이 나열합니다.
...
> [중요] 응답 필드는 해당 그리드의 모든 컬럼에 대응하는 필드를 빠짐없이 나열하세요. 주요 항목만 나열하지 마세요.
...
17. Part 4(API 설계): 화면의 버튼 동작, 조회/저장 흐름, 필터 파라미터, 그리드 컬럼에서 필요한 API를 추론하세요. 엔드포인트 경로는 RESTful 컨벤션을 따르되, 실제 구현과 다를 수 있음을 전제합니다. 요청/응답 필드명은 코드의 state, columnDefs, API 호출 파라미터에서 추론하세요
18. [중요] 드롭다운/코드 목록 정의 섹션에서 모든 선택형 항목(드롭다운, 라디오)의 코드 목록을 빠짐없이 작성하세요. 예를 들어 필터바에 드롭다운이 5개면 코드 목록도 5개 작성해야 합니다. 1~2개만 작성하고 나머지를 생략하는 것을 금지합니다
19. [중요] 조회 API의 응답 필드 테이블은 해당 그리드의 모든 컬럼에 대응하는 필드를 포함해야 합니다. 그리드가 23컬럼이면 응답 필드도 최소 23개 이상 나열하세요. "주요 항목"이라고 하면서 일부만 나열하는 것을 금지합니다
20. [중요] API 엔드포인트 목록에 나열한 모든 API에 대해 조회 API 상세 또는 저장/수정 API 상세를 각각 작성하세요. 엔드포인트 목록만 나열하고 상세를 생략하는 것을 금지합니다
21. 필터바의 조회 파라미터는 API 요청 파라미터와 1:1로 매핑되어야 합니다. 필터 항목이 10개면 요청 파라미터도 10개 이상 나열하세요
...
22. [완성도 검증] 작성 완료 후 아래 체크리스트를 내부적으로 확인하고, 누락된 항목이 있으면 보완하세요 (체크리스트 자체는 출력하지 마세요):
    - 필터바 드롭다운 수 == 드롭다운/코드 목록 수
    - 그리드 컬럼 수 == 조회 API 응답 필드 수
    - 필터 항목 수 == 조회 API 요청 파라미터 수
    - API 엔드포인트 목록의 모든 API에 상세 스펙이 있는지
    - 체크박스 일괄 처리가 있으면 배치/벌크 처리 규칙이 있는지
    - 상태 배지가 있으면 상태 전이 규칙이 있는지
...
| - | 개인마감 (다이얼로그) | 마감 | 재무파트 전용, 다음 달 예산으로 이월 |
| 마감 | 마감취소 (다이얼로그) | 마감 전 상태 복원 | 재무파트 전용 |
...
| 5 | GET | /budgets/closing | 개인마감 대상 목록 조회 | 개인마감 다이얼로그 오픈 |
| 6 | POST | /budgets/closing | 개인마감 처리 | 개인마감 다이얼로그 마감 버튼 |
| 7 | DELETE | /budgets/closing | 개인마감 취소 | 개인마감 다이얼로그 마감취소 버튼 |
| 8 | GET | /budgets/schedule | 예산일자 조회 | 예산일자관리 다이얼로그 오픈 |
| 9 | POST | /budgets/schedule | 예산일자 저장 | 예산일자관리 다이얼로그 저장 버튼 |
...
| 파라미터 | 타입 | 필수 | 설명 | 비고 |
|---------|------|------|------|------|
| budgetMonth | 날짜 | N | 예산년월 | YYYY-MM, 기본값 당월 |
| limitType | 문자열 | N | 한도 구분 | 전체/회사/부서/개인 |
| deptName | 문자열 | N | 더존부서명 | 부분 일치 검색 |
| registrant | 문자열 | N | 등록자명 | 부분 일치 검색 |
| status | 문자열 | N | 신청/승인 상태 | 전체/신청/승인 |
...
| id | 문자열 | 예산 건 고유 ID | - |
| status | 문자열 | 상태 | 신청/승인 |
| limitType | 문자열 | 구분 | 개인/회사/부서 |
| deptCode | 문자열 | 더존코드 | 네 자리 숫자 |
| deptName | 문자열 | 부서/개인명 | - |
| budgetMonth | 문자열 | 예산년월 | YYYY-MM |
| summary | 문자열 | 적요 | - |
| amount | 숫자 | 금액 | - |
| registeredAt | 날짜 | 등록일 | YYYY-MM-DD |
| registrant | 문자열 | 등록자 | - |
| approver | 문자열 | 승인자 | - |
| approvedAt | 날짜 | 승인일시 | YYYY-MM-DD HH:MM:SS |
