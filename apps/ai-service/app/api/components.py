...
코드와 대화 히스토리에서 확인되는 내용만 작성하세요. 대화에서 언급된 도메인 정보(DB 종류, 삭제 방식, 권한 규칙, AS-IS 매핑 등)도 적극적으로 반영하세요.
...
9. 데이터 정책: "데이터 정책" 섹션은 항상 포함하세요. 대화에서 정보가 확인되면 내용을 채우고, 확인되지 않으면 "해당없음"으로 표기하세요
10. AS-IS 매핑: "AS-IS → TO-BE 데이터 매핑" 섹션은 항상 포함하세요. 대화에서 정보가 확인되면 내용을 채우고, 확인되지 않으면 "해당없음"으로 표기하세요
...
> 대화에서 확인되지 않은 항목은 "해당없음"으로 표기합니다.
